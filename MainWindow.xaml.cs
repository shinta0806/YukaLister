...
// ----------------------------------------------------------------------------
// ToDo:
// ゆかり用リストの時だけ、最上段にゆかりへのメニュー「検索」「予約一覧」
// 逆に、ディレクトリーからは「ゆかり検索」を削除
// ディレクトリーの「カテゴリー別」とかをクリッカブルに
// インポートにドロップした時に R:\\ のようにダブる。
...
namespace YukaLister
{
	...
	public partial class MainWindow : Window
	{
		...
		// --------------------------------------------------------------------
		// 自ウィンドウ宛へメッセージを送信
		// 非 UI スレッドから送信する場合でも Dispatcher.Invoke() されている必要はない
		// --------------------------------------------------------------------
		...
		private void RunSyncClientIfNeeded(Boolean oIsReget = false)
		{
			...
			SyncClient aSyncClient = new SyncClient(mYukaListerSettings, LabelBgStatus, mClosingCancellationTokenSource.Token, oIsReget);
		...
		private IntPtr WndProc(IntPtr oHWnd, Int32 oMsg, IntPtr oWParam, IntPtr oLParam, ref Boolean oHandled)
		{
			...
			switch ((Wm)oMsg)
			{
				...
				case (Wm)WindowsApi.WM_SHNOTIFY:
					...
					break;
				case Wm.LaunchFolderTaskRequested:
					WmLaunchFolderTaskRequested();
					break;
				case Wm.LaunchListTaskRequested:
					WmLaunchListTaskRequested();
					break;
				case Wm.UpdateDataGridItemSourceRequested:
					WmUpdateDataGridItemSourceRequested();
					break;
				case Wm.UpdateYukaListerStatusRequested:
		...
		private void DataGridTargetFolders_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
		{
			try
			{
				Int32 aRowIndex;
				Int32 aColumnIndex;
				Common.GetDataGridCellPosition(DataGridTargetFolders, e.MouseDevice.DirectlyOver as DependencyObject, out aRowIndex, out aColumnIndex);
		...
		private void MenuItemAbout_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				AboutWindow aAboutWindow = new AboutWindow(mLogWriter);
				aAboutWindow.Owner = this;
				aAboutWindow.ShowDialog();
			}
			catch (Exception oExcep)
			{
		...
		private void ButtonTFounds_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				DateTime aMusicInfoDbTimeBak = new FileInfo(YlCommon.MusicInfoDbPath()).LastWriteTime;

				ViewTFoundsWindow aViewTFoundsWindow = new ViewTFoundsWindow(mYukaListerSettings, mLogWriter);
				aViewTFoundsWindow.Owner = this;
				aViewTFoundsWindow.ShowDialog();
		...
		private void ButtonYukaListerSettings_Click(object sender, RoutedEventArgs e)
		{
			try
			{
