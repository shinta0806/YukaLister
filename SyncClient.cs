...
namespace YukaLister.Shared
{
	public class SyncClient
	{
		...
		public SyncClient(YukaListerSettings oYukaListerSettings, ToolStripStatusLabel oStatusLabel, CancellationToken oCancellationToken, Boolean oIsReget = false)
		{
			...
			mLogWriterSync.ApplicationQuitToken = mCancellationToken;
			...
			mLogWriterSyncDetail.ApplicationQuitToken = mCancellationToken;
		...
		{
			return YlCommon.LaunchTaskAsync<Object>(SyncMusicInfoDbByWorker, mTaskLockSync, null);
		}
		...
		private void DownloadRejectDate()
		{
			try
			{
				...
				if (aRejectMjd < mYukaListerSettings.LastSyncDownloadDate)
				{
					mYukaListerSettings.LastSyncDownloadDate = aRejectMjd;
				}
			}
		...
		private void DownloadSyncData(out Int32 oNumTotalDownloads, out Int32 oNumTotalImports)
		{
			...
			if (mYukaListerSettings.LastSyncDownloadDate < YlCommon.INVALID_MJD)
			{
			...
			for (; ; )
			{
				...
				{
					// 今日を超えたら抜ける（mYukaListerSettings.LastSyncDownloadDate は今日の日付のままにしておく）
					break;
				}
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (Dictionary<String, String> aOneData in oSyncData)
					{
						...
						// TBase
						aDbNewRecord.SongId = YlCommon.NormalizeDbString(aOneData[TArrangerSequence.FIELD_NAME_ARRANGER_SEQUENCE_ID]);
						...
						aDbNewRecord.PersonId = YlCommon.NormalizeDbString(aOneData[TArrangerSequence.FIELD_NAME_ARRANGER_SEQUENCE_LINK_ID]);

						// ID・連番 が既にテーブル内にあるか確認
						TArrangerSequence aDbExistRecord = aTableArrangerSequence.SingleOrDefault(x => x.SongId == aDbNewRecord.SongId && x.Sequence == aDbNewRecord.Sequence);
						if (aDbExistRecord == null)
						{
							// 新規登録（TSequence は紐付きの増減で無効データが生成され、ローカルデータと競合する可能性があるため、無効データも含めて登録する）
							aTableArrangerSequence.InsertOnSubmit(aDbNewRecord);
							mLogWriterSyncDetail.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "新規登録：" + aDbNewRecord.SongId + " / " + aDbNewRecord.Sequence);
							aNumImports++;
						}
						else
						{
							if (aDbExistRecord.UpdateTime == aDbNewRecord.UpdateTime)
							{
								...
								mLogWriterSyncDetail.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "更新不要：" + aDbNewRecord.SongId + " / " + aDbNewRecord.Sequence);
							}
							else
							{
								// 更新日時がサーバーと異なる場合はそのままではアップロードできないのでサーバーデータで上書きする
								Common.ShallowCopy(aDbNewRecord, aDbExistRecord);
								mLogWriterSyncDetail.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "更新：" + aDbNewRecord.SongId + " / " + aDbNewRecord.Sequence);
								aNumImports++;
							}
						}
						mCancellationToken.ThrowIfCancellationRequested();
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (Dictionary<String, String> aOneData in oSyncData)
					{
						...
						// TBase
						aDbNewRecord.SongId = YlCommon.NormalizeDbString(aOneData[TArtistSequence.FIELD_NAME_ARTIST_SEQUENCE_ID]);
						...
						aDbNewRecord.PersonId = YlCommon.NormalizeDbString(aOneData[TArtistSequence.FIELD_NAME_ARTIST_SEQUENCE_LINK_ID]);

						// ID・連番 が既にテーブル内にあるか確認
						TArtistSequence aDbExistRecord = aTableArtistSequence.SingleOrDefault(x => x.SongId == aDbNewRecord.SongId && x.Sequence == aDbNewRecord.Sequence);
						if (aDbExistRecord == null)
						{
							// 新規登録（TSequence は紐付きの増減で無効データが生成され、ローカルデータと競合する可能性があるため、無効データも含めて登録する）
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (Dictionary<String, String> aOneData in oSyncData)
					{
						...
						// TBase
						aDbNewRecord.SongId = YlCommon.NormalizeDbString(aOneData[TComposerSequence.FIELD_NAME_COMPOSER_SEQUENCE_ID]);
						...
						aDbNewRecord.PersonId = YlCommon.NormalizeDbString(aOneData[TComposerSequence.FIELD_NAME_COMPOSER_SEQUENCE_LINK_ID]);

						// ID・連番 が既にテーブル内にあるか確認
						TComposerSequence aDbExistRecord = aTableComposerSequence.SingleOrDefault(x => x.SongId == aDbNewRecord.SongId && x.Sequence == aDbNewRecord.Sequence);
						if (aDbExistRecord == null)
						{
							// 新規登録（TSequence は紐付きの増減で無効データが生成され、ローカルデータと競合する可能性があるため、無効データも含めて登録する）
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (Dictionary<String, String> aOneData in oSyncData)
					{
						...
						// TBase
						aDbNewRecord.SongId = YlCommon.NormalizeDbString(aOneData[TLyristSequence.FIELD_NAME_LYRIST_SEQUENCE_ID]);
						...
						aDbNewRecord.PersonId = YlCommon.NormalizeDbString(aOneData[TLyristSequence.FIELD_NAME_LYRIST_SEQUENCE_LINK_ID]);

						// ID・連番 が既にテーブル内にあるか確認
						TLyristSequence aDbExistRecord = aTableLyristSequence.SingleOrDefault(x => x.SongId == aDbNewRecord.SongId && x.Sequence == aDbNewRecord.Sequence);
						if (aDbExistRecord == null)
						{
							// 新規登録（TSequence は紐付きの増減で無効データが生成され、ローカルデータと競合する可能性があるため、無効データも含めて登録する）
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (Dictionary<String, String> aOneData in oSyncData)
					{
						...
						// TBase
						aDbNewRecord.TieUpId = YlCommon.NormalizeDbString(aOneData[TTieUpGroupSequence.FIELD_NAME_TIE_UP_GROUP_SEQUENCE_ID]);
						...
						aDbNewRecord.TieUpGroupId = YlCommon.NormalizeDbString(aOneData[TTieUpGroupSequence.FIELD_NAME_TIE_UP_GROUP_SEQUENCE_LINK_ID]);

						// ID・連番 が既にテーブル内にあるか確認
						TTieUpGroupSequence aDbExistRecord = aTableTieUpGroupSequence.SingleOrDefault(x => x.TieUpId == aDbNewRecord.TieUpId && x.Sequence == aDbNewRecord.Sequence);
						if (aDbExistRecord == null)
						{
							// 新規登録（TSequence は紐付きの増減で無効データが生成され、ローカルデータと競合する可能性があるため、無効データも含めて登録する）
							aTableTieUpGroupSequence.InsertOnSubmit(aDbNewRecord);
							mLogWriterSyncDetail.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "新規登録：" + aDbNewRecord.TieUpId + " / " + aDbNewRecord.Sequence);
							aNumImports++;
						}
						else
						{
							...
							{
								// 更新日時がサーバーと同じ場合は同期に支障ないので更新しない（ローカルで編集中の場合はローカルの編集が生きることになる）
								mLogWriterSyncDetail.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "更新不要：" + aDbNewRecord.TieUpId + " / " + aDbNewRecord.Sequence);
							}
							else
							{
								// 更新日時がサーバーと異なる場合はそのままではアップロードできないのでサーバーデータで上書きする
								Common.ShallowCopy(aDbNewRecord, aDbExistRecord);
								mLogWriterSyncDetail.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "更新：" + aDbNewRecord.TieUpId + " / " + aDbNewRecord.Sequence);
								aNumImports++;
							}
						}
						mCancellationToken.ThrowIfCancellationRequested();
		...
		private void LoginToSyncServer()
		{
			...
			{
				// HTML Name 属性
				{ "Name", mYukaListerSettings.SyncAccount },
				{ "PW", YlCommon.Decrypt(mYukaListerSettings.SyncPassword) },
...
#if DEBUGz
			// クッキーの値を抜き出す
			CookieCollection cookies = oDownloader.ClientHandler.CookieContainer.GetCookies(new Uri(mYukaListerSettings.SyncServer));
		...
		private void Post(Dictionary<String, String> oPostParams, Dictionary<String, String> oFiles = null)
		{
			try
			{
				mDownloader.Post(mYukaListerSettings.SyncServer + FILE_NAME_CP_MAIN, oPostParams, oFiles);
				Thread.Sleep(SYNC_INTERVAL);
				mCancellationToken.ThrowIfCancellationRequested();
			}
			catch (Exception)
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (TArrangerSequence aArrangerSequence in aQueryResult)
					{
						...
						// TBase（Dirty を除く）
						aRecord.Add(aArrangerSequence.SongId);
						...
						aRecord.Add(aArrangerSequence.PersonId);
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (TArtistSequence aArtistSequence in aQueryResult)
					{
						...
						// TBase（Dirty を除く）
						aRecord.Add(aArtistSequence.SongId);
						...
						aRecord.Add(aArtistSequence.PersonId);
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (TComposerSequence aComposerSequence in aQueryResult)
					{
						...
						// TBase（Dirty を除く）
						aRecord.Add(aComposerSequence.SongId);
						...
						aRecord.Add(aComposerSequence.PersonId);
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (TLyristSequence aLyristSequence in aQueryResult)
					{
						...
						// TBase（Dirty を除く）
						aRecord.Add(aLyristSequence.SongId);
						...
						aRecord.Add(aLyristSequence.PersonId);
			...
			using (SQLiteConnection aConnection = YlCommon.CreateMusicInfoDbConnection())
			{
				using (DataContext aContext = new DataContext(aConnection))
				{
					...
					foreach (TTieUpGroupSequence aTieUpGroupSequence in aQueryResult)
					{
						...
						// TBase（Dirty を除く）
						aRecord.Add(aTieUpGroupSequence.TieUpId);
						...
						aRecord.Add(aTieUpGroupSequence.TieUpGroupId);
		...
		private void SyncMusicInfoDbByWorker(Object oDummy)
		{
			...
			{
				SetStatusLabelMessageWithInvoke(TraceEventType.Error, "楽曲情報データベース同期タスク実行時エラー：" + oExcep.Message);
		...
		{
			return mYukaListerSettings.SyncServer + FILE_NAME_CP_MAIN + "?Mode=" + oMode;
		}
