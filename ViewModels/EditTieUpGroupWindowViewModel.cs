...
{
	public class EditTieUpGroupWindowViewModel : EditMasterWindowViewModel
	{
		/* コマンド、プロパティの定義にはそれぞれ 
         * 
		...
		public override void Initialize()
		{
			...
			try
			{
				...
				// ヒント
				NameHint = "シリーズ名に「" + YlCommon.TIE_UP_GROUP_SUFFIX + "」は含めないで下さい。";

			}
			catch (Exception oExcep)
			{
				Environment.LogWriter.ShowLogMessage(TraceEventType.Error, "シリーズ詳細情報編集ウィンドウビューモデル初期化時エラー：\n" + oExcep.Message);
		...
		protected override String Save()
		{
			...
			using (DataContext aContext = new DataContext(aMusicInfoDbInDisk.Connection))
			{
				if (aNewRecord.Id == NewIdForDisplay())
				{
					...
					aNewRecord.Id = Environment.YukaListerSettings.PrepareLastId(aMusicInfoDbInDisk.Connection, MusicInfoDbTables.TTieUpGroup);
					Table<TTieUpGroup> aTableTieUpGroup = aContext.GetTable<TTieUpGroup>();
					aTableTieUpGroup.InsertOnSubmit(aNewRecord);
					Environment.LogWriter.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "シリーズテーブル新規登録：" + aNewRecord.Id + " / " + aNewRecord.Name);
				}
				else
				{
					...
					if (YlCommon.IsRcMasterUpdated(aExistRecord, aNewRecord))
					{
						...
						Environment.LogWriter.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "シリーズテーブル更新：" + aNewRecord.Id + " / " + aNewRecord.Name);
					}
				}
		...
		{
			return YlCommon.SelectMastersByName<TTieUpGroup>(oConnection, oName).ToList<IRcMaster>();
		}


	}
	// public class EditTieUpGroupWindowViewModel ___END___
}
