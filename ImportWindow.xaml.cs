...
namespace YukaLister
{
	...
	public partial class ImportWindow : Window
	{
		...
		// コンストラクター
		// --------------------------------------------------------------------
		public ImportWindow(YukaListerSettings oYukaListerSettings, LogWriter oLogWriter)
		{
			InitializeComponent();
		...
		private async void Window_Loaded(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				{
					LabelSrc.Content = "anison.info CSV をインポートします";
					await ImportAnisonInfoAsync();
				}
				else if (IsNicoKaraListerMode)
				{
					LabelSrc.Content = "ニコカラりすたーでエクスポートしたファイルをインポートします";
		...
		private void Window_SourceInitialized(object sender, EventArgs e)
		{
			...
			{
				mLogWriter.ShowLogMessage(TraceEventType.Error, "インポートウィンドウ初期化時エラー：\n" + oExcep.Message);
				mLogWriter.ShowLogMessage(TraceEventType.Verbose, "　スタックトレース：\n" + oExcep.StackTrace);
			}
		}
