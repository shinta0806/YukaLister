...
{
	public class MusicInfoDatabaseCommon
	{
		// ====================================================================
		// public メンバー関数
		// ====================================================================
		...
		// --------------------------------------------------------------------
		public static void SetTFoundValue(TFound oRecord, FolderSettingsInMemory oFolderSettingsInMemory, SQLiteCommand oMusicInfoDbCmd, DataContext oMusicInfoDbContext,
				List<String> oCategoryNames)
		{
			...
			// 楽曲情報データベースを適用
			SetTFoundValueByMusicInfoDb(oRecord, aDicByFile, oMusicInfoDbContext);
		...
		// --------------------------------------------------------------------
		private static void AnalyzeSmartTrack(String oTrack, out Boolean oHasOn, out Boolean oHasOff)
		{
			oHasOn = false;
			oHasOff = false;
		...
		// --------------------------------------------------------------------
		private static void ConcatPersonNameAndRuby(List<TPerson> oPeople, out String oName, out String oRuby)
		{
			if (oPeople.Count == 0)
			{
				oName = null;
		...
		// 比較関数
		// --------------------------------------------------------------------
		private static Int32 ConcatPersonNameAndRubyCompare(TPerson oLhs, TPerson oRhs)
		{
		...
		// --------------------------------------------------------------------
		public static String ProgramOrigin(String oAlias, SQLiteCommand oMusicInfoDbCmd)
		{
		...
		public static String ProgramOrigin(String oAlias, DataContext oContext)
		{
			if (String.IsNullOrEmpty(oAlias))
			{
				return null;
		...
		// --------------------------------------------------------------------
		private static void SetTFoundValueByMusicInfoDb(TFound oRecord, Dictionary<String, String> oDicByFile, DataContext oMusicInfoDbContext)
		{
		...
		// --------------------------------------------------------------------
		private static String SongOrigin(String oAlias, SQLiteCommand oInfoDbCmd)
		{
			if (String.IsNullOrEmpty(oAlias))
			{
				return null;
	...
	// public class MusicInfoDatabaseCommon ___END___
}
